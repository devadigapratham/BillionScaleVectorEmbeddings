#!/usr/bin/env python3
"""
Working vector search benchmarking script that excludes problematic libraries.
This version focuses on stable libraries: FAISS, scikit-learn, and SciPy.
"""
...
# Vector search libraries - only stable ones
try:
    import faiss
...
# Note: HNSWlib is excluded due to segmentation fault issues
...
class WorkingVectorBenchmarker:
    ...
    def measure_build_time(self, func, *args, **kwargs) -> Tuple[float, float, Any]:
    ...
    def run_benchmarks(self):
        ...
        logger.info("Starting comprehensive vector search benchmarks (stable libraries only)")
    ...
    def create_visualizations(self, df: pd.DataFrame):
        ...
        fig.suptitle('Vector Search Benchmark Results (Stable Libraries)', fontsize=16, fontweight='bold')
        ...
        ax.set_title('Benchmark Summary Statistics (Stable Libraries)', fontsize=14, fontweight='bold', pad=20)
    ...
    def generate_report(self, df: pd.DataFrame):
        ...
        with open(report_path, 'w') as f:
            f.write("VECTOR SEARCH BENCHMARK REPORT (Stable Libraries)\n")
            f.write("=" * 60 + "\n\n")
            f.write(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Total benchmarks run: {len(self.results)}\n")
            f.write("Note: HNSWlib excluded due to system compatibility issues\n\n")
            ...
            if not df.empty:
                # For production use
                prod_libs = df[df['library'].isin(['FAISS'])]
...
def main():
    parser = argparse.ArgumentParser(description="Working Vector Search Benchmarking Framework")
    ...
    # Create benchmarker
    benchmarker = WorkingVectorBenchmarker(
